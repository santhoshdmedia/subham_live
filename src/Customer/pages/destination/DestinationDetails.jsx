          ...
          <ul className="list-disc pl-6 space-y-3 text-base text-gray-700">
            {activities.map((activity, idx) => (
              <li key={idx} className="font-semibold">
                <span className="text-primary">{activity.time} : </span> {activity.description}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="w-full font-primary_font">
        ...
        <div className="px-4 py-10 md:px-[6vw]">
          <div className="w-full mb-5">
            <img src={destination.image || IMAGE_HELPER.DefaultPackageImage} alt="Destination" className="w-full h-[365px] object-cover rounded-lg" />
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-5">
            <div className="w-full space-y-6 text-gray-800">
              ...
              <div className="space-y-4">
                <div className="flex flex-col">
                  <div className="flex items-center text-xl font-semibold text-primary">
                    {destination.country === "india" ? "LKR" : "INR"} {Number(destination.discount_price || 0).toFixed(2)}
                    <span className="line-through ml-2 text-gray-400 text-base">
                      {destination.country === "india" ? "LKR" : "INR"} {Number(destination.original_price || 0).toFixed(2)}
                    </span>
                    <span className="ml-2 text-black text-base font-normal">/ Per Person</span>
                  </div>

                  <div className="flex flex-col lg:flex-row gap-4 mt-3">
                    {details.map((detail, index) => (
                      <div className="flex items-center font-pri_head" key={index}>
                        {detail.icon}
                ...
                {/* Description */}
                <div className="font-pri_para text-gray-700  text-base font-normal">{destination.description}</div>
              </div>

              {/* Attractions */}
              <div className="space-y-3">
                <h3 className="text-lg font-pri_head font-semibold text-primary">Top Attractions</h3>
                <ul className="space-y-4 grid grid-cols-1 md:grid-cols-2">
                  {(destination.top_attractions || []).map((attraction, index) => (
                    <li key={index} className="lg:flex lg:items-start items-center  gap-4">
                      <img src={attraction.image} alt={attraction.name} className="w-[150px] h-[100px] object-cover rounded-lg" />
                      <div>
                        <h4 className="flex items-center gap-2 font-semibold">
                          <ICON_HELPER.STAR_ICON className="text-yellow-500" />
                          {attraction.name}
                        </h4>
                        <p className="font-pri_para text-gray-700  text-base font-normal lg:w-[400px]">{attraction.description}</p>
                      </div>
                    </li>
                  ))}
                </ul>
              ...
              {/* Included & Excluded */}
              <div className="space-y-3">
                <h3 className="text-lg font-pri_head font-semibold text-primary">Included and Excluded</h3>
              ...
              {/* Itinerary */}
              <div className="space-y-3">
                <h3 className="text-lg font-pri_head font-semibold text-primary">Itinerary</h3>
                <Collapse
                  bordered={false}
                  defaultActiveKey={["1"]}
